...
# shellcheck disable=SC2086
  curl -sI "${url}" -L ${compression} ${cmd_ext} ${heisenbergCompensator} | sed -n 's/^etag: //Ip' > "${piholeDir}/list.${id}.${domain}.etag"

  if [ -z "$currentetag" ]
	then
		httpCode=$(curl --connect-timeout ${curl_connect_timeout} -s -L ${compression} ${cmd_ext} ${heisenbergCompensator} -w "%{http_code}" "${url}" -o "${listCurlBuffer}" -D "${piholeDir}/list.${id}.${domain}.header" 2> /dev/null)
	else
		httpCode=$(curl --connect-timeout ${curl_connect_timeout} -s -L ${compression} ${cmd_ext} --header "If-None-Match: ${currentetag}" -w "%{http_code}" "${url}" -o "${listCurlBuffer}" -D "${piholeDir}/list.${id}.${domain}.header" 2>> /dev/null)
  fi
